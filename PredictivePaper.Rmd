...
pander(naiveE$mean)
#accuracy 43/21
kable(accuracy(naiveE, salesValidE))
```
...
etsForecastE
#accuracy 35/18
kable(accuracy(etsForecastE, salesValidE))
```
...
seasonRegEForecast
#accuracy 33/21
kable(accuracy(seasonRegEForecast, salesValidE))
```
...
adjustedForecast
kable(accuracy(adjustedForecast, salesValidE))
```
...
kable(accuracy(neuralForecast, salesValidE))
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
allForecasts <- (naiveE$mean + etsForecastE$mean + seasonRegEForecast$mean + neuralForecast$mean)/4
pander(allForecasts)
#MAPE of the averaged forecasts = 20.23421
kable(accuracy(allForecasts, salesValidE))
```

Using the hybridForecasting package in R, we can get an average. I've tried this using equal weights. 

```{r, echo=FALSE, message=FALSE, warning=FALSE}
fit1 <- hybridModel(salesTrainE, weights = "equal")
fc1 <- forecast(fit1, h = validLengthE <- 12 )
fc1
kable(accuracy(fc1$mean, salesValidE))
autoplot(fc1) + ggtitle("Hybrid Forecast Item E \nwith Confidence Intervals") + xlab("Year") +  
  ylab("Item E Use")
df <- cbind(Data=e.ts, Hybrid1=fc1$mean)
autoplot(df) +
xlab("Year") + ylab(expression("Item E Use"))+ ggtitle("Hybrid Forecast Item E \nwith Confidence Intervals")
```
...
plot(e.ts, bty = "n", main = "Actual Use and Forecasting Models \nSS06S6T100144 - Item E", ylim = c(0,800))
...
lines(fc1$mean, col = "purple")


legend(2012, 800, c("Actual", "Naive", "ETS", "Regression", "Neural Net", "Average"), lty = c(1,1,1,1,1, 1), lwd = c(1,1,1,1,1, 1), bty = "n", col = c("black", "blue", "green", "orange", "red", "purple"))
```
...
allForecasts <- (naiveE$mean + etsForecastE$mean  + seasonRegEForecast$mean + neuralForecast$mean)/4
kable(allForecasts)
#MAPE of the averaged forecasts = 11.73
kable(accuracy(allForecasts, salesValidE))
```
...
fc1
kable(accuracy(fc1$mean, salesValidE))
autoplot(fc1) + ggtitle("Hybrid Forecast Item F \nwith Confidence Intervals") + xlab("Year") +  
  ylab("Item F Use")
df <- cbind(Data=f.ts, Hybrid1=fc1$mean)
autoplot(df) +
xlab("Year") + ylab(expression("Item F Use"))+ ggtitle("Hybrid Forecast Item F \nwith Confidence Intervals")
```
...
```{r, message=FALSE, warning=FALSE, include=FALSE}
validLengthE <- 12
trainLengthE <- length(i.ts) - validLengthE
salesTrainE <- window(i.ts, end = c(2012, trainLengthE))
salesValidE <- window(i.ts, start = c(2012, trainLengthE + 1))
#naive
naiveE <- snaive(salesTrainE, h = validLengthE)
#pointFC snaive 22/13 naive 21/14
naiveE$mean
...
#best smoothing model, it chooses an AAN model with an alpha = 0.1e-04 and beta = 1e-04.
etsE <- ets(salesTrainE, model = "ZZZ", restrict = FALSE)
etsE
summary(etsE)
etsForecastE <- forecast(etsE, h = validLengthE, level = 0)
etsForecastE
#forecat accuracy 15.38072%
accuracy(etsForecastE$mean, salesValidE)
...
ARModel <- Arima(seasonTrendE$residuals, order = c(1,0,0))
ARModel
#calculate t stat, see it's not significant, ar1 < 2 and >-2
ARModel$coef["ar1"] / 0.1349
#residuals of residuals
Acf(ARModel$residuals)
#forecast residuals
errorForecast <- forecast(ARModel, h = validLengthE)
...
#p=11, MAPE = 0.33/22.88

ARModel <- Arima(neuralE$residuals, order = c(1,0,0))
ARModel
#calculate t stat, see it's not statistically significant, ar1 <2
ARModel$coef["ar1"] / 0.1524
...
# Not improved Forecast MAPE = 22.89
adjustedForecast <- neuralForecast$mean + errorForecast$mean
adjustedForecast
accuracy(adjustedForecast, salesValidE)
```
...
allForecasts <- (naiveE$mean + etsForecastE$mean  + seasonRegEForecast$mean + neuralForecast$mean)/4
kable(allForecasts)
#MAPE of the averaged forecasts = 17.3
kable(accuracy(allForecasts, salesValidE))
```
...
fc1
kable(accuracy(fc1$mean, salesValidE))
autoplot(fc1) + ggtitle("Hybrid Forecast Item I \nwith Confidence Intervals") + xlab("Year") +  
  ylab("Item I Use")
df <- cbind(Data=i.ts, Hybrid1=fc1$mean)
autoplot(df) +
xlab("Year") + ylab(expression("Item I Use"))+ ggtitle("Hybrid Forecast Item I \nwith Confidence Intervals")
```
...
plot(i.ts, bty = "n", main = "Actual Use and Forecasting Models \nSS06S6A100144 Item I ")
lines(naiveE$fitted, col = "blue")
lines(naiveE$mean, col = "blue", lty = 2)
lines(etsE$fitted, col = "green")
lines(etsForecastE$mean, col = "green", lty = 3)
...
legend(2012, 500, c("Actual", "Naive", "ETS", "Regression", "Neural Net", "Average"), lty = c(1,1,1,1,1,1), lwd = c(1,1,1,1,1,1), bty = "n", col = c("black", "blue", "green", "orange", "red"))
```
...
```{r, message=FALSE, warning=FALSE, include=FALSE}
validLengthE <- 12
trainLengthE <- length(m.ts) - validLengthE
salesTrainE <- window(m.ts, end = c(2012, trainLengthE))
salesValidE <- window(m.ts, start = c(2012, trainLengthE + 1))
#naive
naiveE <- snaive(salesTrainE, h = validLengthE)
#pointFC snaive 22/13 naive 21/14
naiveE$mean
...
#best smoothing model, it chooses an MAN model with an alpha = 0.1e-04 and beta = 1e-04.
etsE <- ets(salesTrainE, model = "ZZZ", restrict = FALSE)
etsE
summary(etsE)
etsForecastE <- forecast(etsE, h = validLengthE, level = 0)
...
residualsseasonTrendE <- Acf(seasonTrendE$residuals, main = "Some Seasonality \nLeft to Capture")
#There's uncaptured T/S
ARModel <- Arima(seasonTrendE$residuals, order = c(1,0,0))
ARModel
#calculate t stat, see it's significant, ar1 > 2, ar-1 = 3.539817
ARModel$coef["ar1"] / 0.1247
#residuals of residuals
Acf(ARModel$residuals)
#forecast residuals
errorForecast <- forecast(ARModel, h = validLengthE)
errorForecast
#improved Forecast MAPE = 28.51856
adjustedForecast <- seasonRegEForecast$mean + errorForecast$mean
adjustedForecast
accuracy(adjustedForecast, salesValidE)
...
ARModel <- Arima(neuralE$residuals, order = c(1,0,0))
ARModel
#calculate t stat, see it's statistically significant, ar1 < - 2
ARModel$coef["ar1"] / 0.1528

#residuals of residuals
Acf(ARModel$residuals)
#forecast residuals
errorForecastN <- forecast(ARModel, h = validLengthE)
errorForecastN

# Not much improved Forecast MAPE = 22.92979
adjustedForecastN <- neuralForecast$mean + errorForecast$mean
adjustedForecastN
accuracy(adjustedForecastN, salesValidE)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
kable(accuracy(adjustedForecastN, salesValidE))
```
...
fc1
kable(accuracy(fc1$mean, salesValidE))
autoplot(fc1) + ggtitle("Hybrid Forecast Item M \nwith Confidence Intervals") + xlab("Year") +  
  ylab("Item M Use")
df <- cbind(Data=m.ts, Hybrid1=fc1$mean)
autoplot(df) +
xlab("Year") + ylab(expression("Item E Use"))+ ggtitle("Hybrid Forecast Item M \nwith Confidence Intervals")
```
...
plot(m.ts, bty = "n", main = "Actual Use and Forecasting Models \nSS03S6T7284 Item M ", ylim = c(0,300))
lines(naiveE$fitted, col = "blue")
lines(naiveE$mean, col = "blue", lty = 2)
lines(etsE$fitted, col = "green")
lines(etsForecastE$mean, col = "green", lty = 3)
...
legend(2012, 300, c("Actual", "Naive", "ETS", "Regression", "Neural Net", "Average"), lty = c(1,1,1,1,1,1), lwd = c(1,1,1,1,1,1), bty = "n", col = c("black", "blue", "green", "orange", "red"))
```
...
```{r, message=FALSE, warning=FALSE, include=FALSE}
validLengthE <- 12
trainLengthE <- length(n.ts) - validLengthE
salesTrainE <- window(n.ts, end = c(2012, trainLengthE))
salesValidE <- window(n.ts, start = c(2012, trainLengthE + 1))
#naive
naiveE <- snaive(salesTrainE, h = validLengthE)
#pointFC snaive 17/13 naive 21/14
naiveE$mean
#accuracy 43/24
accuracy(naiveE, salesValidE)

#best smoothing model, it chooses an MNA model with an alpha = 0.1757  and gamma = 2e-04.
etsE <- ets(salesTrainE, model = "ZZZ", restrict = FALSE)
etsE
summary(etsE)
etsForecastE <- forecast(etsE, h = validLengthE, level = 0)
etsForecastE
#forecat accuracy 10.75%
accuracy(etsForecastE$mean, salesValidE)
...
ARModel <- Arima(seasonTrendE$residuals, order = c(1,0,0))
ARModel
#calculate t stat, see it's significant, ar1 > 2, ar-1 = 2.452
ARModel$coef["ar1"] / 0.1305
#residuals of residuals
Acf(ARModel$residuals)
#forecast residuals
errorForecast <- forecast(ARModel, h = validLengthE)
errorForecast
#improved Forecast MAPE = 10.34
adjustedForecast <- seasonRegEForecast$mean + errorForecast$mean
adjustedForecast
accuracy(adjustedForecast, salesValidE)
...
neuralForecast$mean
accuracy(neuralForecast, salesValidE)
#p=11, MAPE = 0.13/13.01

ARModelN <- Arima(neuralE$residuals, order = c(1,0,0))
ARModelN
#calculate t stat, see it's statistically significant, ar1 < - 2
ARModelN$coef["ar1"] / 0.1562
...
# Not much improved Forecast MAPE = 13.12
adjustedForecastN <- neuralForecast$mean + errorForecast$mean
adjustedForecastN
accuracy(adjustedForecastN, salesValidE)
```
...
allForecasts <- (naiveE$mean + etsForecastE$mean  + seasonRegEForecast$mean + neuralForecast$mean)/4
pander(allForecasts)
#MAPE of the averaged forecasts = 9.23
kable(accuracy(allForecasts, salesValidE))
```
...
fc1
kable(accuracy(fc1$mean, salesValidE))
autoplot(fc1) + ggtitle("Hybrid Forecast Item N \nwith Confidence Intervals") + xlab("Year") +  
  ylab("Item N Use")
df <- cbind(Data=n.ts, Hybrid1=fc1$mean)
autoplot(df) +
xlab("Year") + ylab(expression("Item N Use"))+ ggtitle("Hybrid Forecast Item N \nwith Confidence Intervals")
```
This plot shows the original series in black; it is obscured by the neural net fit model. The forecasts are in dotted lines. All validation forecasts under forecast. 

```{r, echo=FALSE, message=FALSE, warning=FALSE}

plot(n.ts, bty = "n", main = "Actual Use and Forecasting Models \nSS03S6T7284 Item M ", xlim = c(2012,2019))
lines(naiveE$fitted, col = "blue")
lines(naiveE$mean, col = "blue", lty = 2)
lines(etsE$fitted, col = "green")
lines(etsForecastE$mean, col = "green", lty = 3)
...
legend(2017, 350, c("Actual", "Naive", "ETS", "Regression", "Neural Net", "Average"), lty = c(1,1,1,1,1,1), lwd = c(1,1,1,1,1,1), bty = "n", col = c("black", "blue", "green", "orange", "red"))
```
...
```{r, message=FALSE, warning=FALSE, include=FALSE}
validLengthE <- 12
trainLengthE <- length(s.ts) - validLengthE
salesTrainE <- window(s.ts, end = c(2012, trainLengthE))
salesValidE <- window(s.ts, start = c(2012, trainLengthE + 1))
#naive
naiveE <- snaive(salesTrainE, h = validLengthE)
#pointFC snaive 43/21
naiveE$mean
#accuracy 43/24
accuracy(naiveE, salesValidE)

#best smoothing model, it chooses an ANN model with an alpha = 1e-04 .
etsE <- ets(salesTrainE, model = "ZZZ", restrict = FALSE)
etsE
summary(etsE)
etsForecastE <- forecast(etsE, h = validLengthE, level = 0)
...
fc1
kable(accuracy(fc1$mean, salesValidE))
autoplot(fc1) + ggtitle("Hybrid Forecast Item S \nwith Confidence Intervals") + xlab("Year") +  
  ylab("Item S Use")
df <- cbind(Data=s.ts, Hybrid1=fc1$mean)
autoplot(df) +
xlab("Year") + ylab(expression("Item S Use"))+ ggtitle("Hybrid Forecast Item S \nwith Confidence Intervals")
```
...
plot(s.ts, bty = "n", main = "Actual Use and Forecasting Models \nSS03S6T7284 Item M ", xlim = c(2012,2019))
lines(naiveE$fitted, col = "blue")
lines(naiveE$mean, col = "blue", lty = 2)
lines(etsE$fitted, col = "green")
lines(etsForecastE$mean, col = "green", lty = 3)
...
legend(2017, 150, c("Actual", "Naive", "ETS", "Regression", "Neural Net", "Average"), lty = c(1,1,1,1,1,1), lwd = c(1,1,1,1,1,1), bty = "n", col = c("black", "blue", "green", "orange", "red"))
```
