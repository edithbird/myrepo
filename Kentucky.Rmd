...
x
p <- ggplot(data = x, aes(x = Year_Number, y = Age, group = State.County, color = State.County)) + geom_line(size = 0.5) + facet_wrap(~ Gender) +
  ggtitle("Kentucky Average Age at Death \nAs Compared to the Rest of the Country") + geom_ribbon(aes(x=Year_Number, ymin=ymin, ymax=ymax), fill="grey", alpha=.4) + scale_color_manual(values=c("#CC6666", "#9999CC")) + geom_smooth(method = "lm", se = FALSE, size = 1.5) 
p
