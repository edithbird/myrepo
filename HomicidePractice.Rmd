...
```{r}
#32635
BlackonWhite <- Homicide %>% filter(Perpetrator.Race == "Black" & Victim.Race=="White") %>% select(Date, City, State, Perpetrator.Race, Victim.Race, Weapon)
head(BlackonWhite)
dim(BlackonWhite)
byWeapBonW <- group_by(BlackonWhite, Weapon)
sumWeapBonW <- summarise(byWeapBonW, count = n())
```


```{r}
#17004
WhiteonBlack <- Homicide %>% filter(Perpetrator.Race == "White" & Victim.Race=="Black") %>% select(Date, City, State, Perpetrator.Race, Victim.Race, Weapon) 
head(WhiteonBlack)
dim(WhiteonBlack)
